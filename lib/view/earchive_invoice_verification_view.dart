...
import '../view_model/earchive_invoice_verification_view_model.dart';
...
class EArchiveInvoiceVerificationView extends StatefulWidget {
  const EArchiveInvoiceVerificationView({super.key});

  @override
  State<EArchiveInvoiceVerificationView> createState() =>
      _EArchiveInvoiceVerificationViewState();
}

class _EArchiveInvoiceVerificationViewState
    extends State<EArchiveInvoiceVerificationView>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      if (await checkConnection()) {
        initWebView(webViewController, VerifyServices.eArchiveInvoice.baseUrl);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(VerifyServices.eArchiveInvoice.name),
      ),
    ...
    return Row(
      ...
      children: [
        ...
        Expanded(
            ...
            child: CustomTextField(
                isNumberType: false,
                text: permanentText.textFieldLabelNameInvoiceNo,
    ...
    return Row(
      children: [
        Expanded(
          ...
          child: CustomTextField(
              isNumberType: true,
              text: permanentText.textFieldLabelNameAmount,
    ...
    queryHtmlElement(htmlObject, javasScriptCommand.querySuccessfulHtmlElement)
      ...
      if (successfulElement == null) {
        queryHtmlElement(htmlObject, javasScriptCommand.queryErrorHtmlElement)
          ...
          successfulElement.text.contains(errorMessageText.formNotRegisteredInvoice)) {
        _animationPath = AnimationType.unsuccessful.path;
  ...
  Future<void> _scanQrCode() async {
    EArchiveInvoiceVerificationModel result = await scanQrCode();

    setState(() {
      vknTckn.text = result.vkntckn;
      amount.text = result.odenecek;
...
Future<void> _setWebViewAttributeValue() async {
  ...
  jsInvoiceId = elementAndAttributeCommand.setElementNameAndValue(
      elementName.invoiceId, invoiceNo.text);
...
//Ornek fatura bilgileri
//VKNTCKN: 21106818108
//Fatura No: EAR2023000000010
//Amount: 3680
