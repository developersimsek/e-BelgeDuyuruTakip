...
class ELedgerListViewModel {
  final DatabaseReference ?reference =
      fireBaseInstance(ListServices.eLedger.servicePath);

  List<ListItemModel> eLedgerList = [];
...
Future<List<ListItemModel>> getELedgerList(DatabaseReference reference) async {
  List<ListItemModel> getELedgerList = [];
    ...
    if (data != null) {
      if (data is String) {
        ...
        getDataMap.forEach((key, value) {
          ...
          getELedgerList.add(ListItemModel.fromJson(itemMap,key));
        });
      }
    }
  });

  getELedgerList.sort((a, b) {
    return b.Date.compareTo(a.Date);
  });

  return getELedgerList;
}
